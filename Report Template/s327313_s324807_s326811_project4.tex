...
The original model was proposed by \cite{chen2017deeplab}
...
720×720 pixels. The performance metric used for evaluation were the mean Intersection over Union (mIoU), the latency as time (in seconds) required to process a single batch during validation, FLOPs to estimate the total number of floating-point operations required for a forward pass through the network, Parameters as the total number of trainable parameters in the model.
...
We applied the Prototype-based Efficient MaskFormer (PEM) architecture to the LoveDA \cite{wang2021loveda} dataset to evaluate its performance in a domain adaptation setting. Specifically, PEM was trained on the LoveDA-Urban dataset, leveraging its urban domain imagery as the source domain, and tested on the LoveDA-Rural dataset, which represents the target domain. This experimental setup allowed us to assess the model's ability to generalize across domains with distinct characteristics, such as differences in class distributions, object scales, and background complexity. The results highlight PEM's potential in addressing domain adaptation challenges in real-time semantic segmentation tasks, demonstrating its capacity to maintain competitive accuracy while operating efficiently in cross-domain scenarios. This study underlines the importance of exploring lightweight and effective architectures for segmentation tasks that require robustness to domain shifts.
